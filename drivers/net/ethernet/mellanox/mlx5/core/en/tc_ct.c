...
static int
mlx5_tc_ct_block_flow_offload_repalce(struct mlx5_ct_ft *ft, struct flow_rule *flow_rule,
				      struct mlx5_ct_entry *entry, unsigned long cookie)
{
	...
	if (err) {
		/* Just released our reference to the entry. On error,
		 * look it up again under ht_lock protection and
		 * properly delete it.
		 */
		spin_lock_bh(&ct_priv->ht_lock);
		entry = rhashtable_lookup_fast(&ft->ct_entries_ht, &cookie,
					       cts_ht_params);
		if (entry) {
			rhashtable_remove_fast(&ft->ct_entries_ht, &entry->node,
					       cts_ht_params);
			spin_unlock_bh(&ct_priv->ht_lock);
				  ...
				  struct flow_cls_offload *flow)
{
	...
	if (entry && refcount_inc_not_zero(&entry->refcnt)) {
		...
		return mlx5_tc_ct_block_flow_offload_repalce(ft, flow_rule, entry, cookie);
	}
	spin_unlock_bh(&ct_priv->ht_lock);
